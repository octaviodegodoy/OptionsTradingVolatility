...
df = mt5_conn.get_data("PETR4", mt5_conn.TIMEFRAME_D1, 55,0)
...
# Scale back the conditional volatility by dividing by 100
annualized_vol = (results.conditional_volatility[-1] / 100) * np.sqrt(252)
...
# Add conditional volatility trace (scale back to original units)
fig.add_trace(go.Scatter(x=dates, y=results.conditional_volatility / 100, mode='lines', name='Conditional Volatility', yaxis='y2'))
